...
Your inputs will be used by a content creation team to produce a high quality to accompany the social media post.
...
NEGATIVE_PROMPT_FOR_IMAGEN = "((((ugly)))), (((duplicate))), ((morbid)), ((mutilated)), swimsuit, text, out of frame, extra fingers, mutated hands, ((poorly drawn hands)), ((poorly drawn face)), (((mutation))), (((deformed))), ((ugly)), blurry, ((bad anatomy)), (((text))), (((bad proportions))), ((extra limbs)), cloned face, (((disfigured))), out of frame, ugly, extra limbs, (bad anatomy), gross proportions, (malformed limbs), ((missing arms)), ((missing legs)), (((extra arms))), (((extra legs))), mutated hands, (fused fingers), (too many fingers), (((long neck)))"
...
**5.  Call to Action (Optional but Recommended):**

* Include a clear and concise call to action if appropriate (e.g., "Learn more," "Join us," "Visit [link]").

**Example incorporating the above for the provided User Input:**
...
def download_folder_from_gcs(bucket_name, gcs_folder, local_folder, project_id):
    ...
    for blob in blobs:
        ...
        local_file_path = os.path.join(local_folder, blob.name[len(gcs_folder):].lstrip('/'))
        os.makedirs(os.path.dirname(local_file_path), exist_ok=True)  # Create directories if they don't exist
...
def get_font_size(textarea, text, font_name, pixel_gap=2):
    ...
    for point_size in range(5, 300):  # Iterate to find ideal font size
        ...
        if total_height >= text_height:
            while (point_size > 3 and (max(line_widths) >= text_width or total_height >= text_height)):
                point_size -= 1
                font = ImageFont.truetype(font_name, point_size)
                wrapped_lines, line_widths = wrap_text_custom(text, font, text_width)
                total_height = (ascent + descent + pixel_gap) * len(wrapped_lines) - pixel_gap
...
def place_multiline_text_overlay_on_background(
    overlay_image_path, text, overlay_config, font_path, text_color, alignment, margin=5
):
    x, y = overlay_config["x"], overlay_config["y"]
    width, height = overlay_config["width"], overlay_config["height"]
    ...
    with Image.open(overlay_image_path) as img:
        ...
        wrapped_lines, font_size = get_font_size(
            (width, height), text, font_path, margin
        )
...
def generate_image_assets(
    ...
    prompt_rewrite = f"""
        ...
        Also ensure all subjects are fully within the frame, avoiding any cropping of their bodies at the edges. It is acceptable to frame the image from the waist up, or even closer, to prioritize full inclusion of the subjects.
    ...
    for index, image_list in enumerate(image_lists):
        for generated_image in image_list:
            ...
            upload_blob(
                GCS_INPUT_BUCKET_ROOT, image_filename, gcs_path, PROJECT_ID
            )
...
def generate_post_text(post_text_prompt_input):
    ...
    output = response.text
    json_output = json.loads(output)
    headline = json_output.get("headline", "Error")
    event_details = json_output.get("event_details", "Error")
    call_to_action = json_output.get("call_to_action", "Error")
...
def download_fonts_and_graphics():
    try:
        logger.info("Downloading fonts and graphics")
        # Download fonts from GCS to temporary location for use in text overlay:
        download_folder_from_gcs(
            GCS_INPUT_BUCKET_ROOT, "Artefacts/Fonts", LOCAL_INPUT_DIR_FONTS, PROJECT_ID
        )

        download_folder_from_gcs(
            GCS_INPUT_BUCKET_ROOT, "Artefacts/Graphics", LOCAL_INPUT_DIR_GRAPHICS, PROJECT_ID
        )
        logger.info("Fonts and graphics downloaded")
    except Exception as e:
        logger.error(f"Error downloading fonts and graphics: {e}")

def process_image(img_file_name, input_text, request_id, idx, aspect_ratio, layouts, primary_font_path, secondary_font_path, disclaimer_and_sign_off, generated_image_urls, user_id):
    no_bg_image_path = os.path.join(LOCAL_TEMP_DIR, f"NoBg_{img_file_name.split('/')[-1]}")
...
def process_new_document(data, context) -> None:
    ...
    new_request = affected_doc.get()
    new_request_dict = new_request.to_dict()
    new_request_dict["requestId"] = affected_doc.id
    new_request = Request(**new_request_dict)

    user_id = new_request.userId
    request_id = new_request.requestId
    post_description = new_request.requestConfig.postDescription
    subject = new_request.requestConfig.subject
    art_style = new_request.requestConfig.artStyle
    sign_off = new_request.requestConfig.signOff
    post_count = new_request.requestConfig.postCount
    aspect_ratio = new_request.requestConfig.aspectRatio
    background_color = new_request.requestConfig.backgroundColor
    is_recruitment_related = new_request.requestConfig.isRecruitmentRelated
    is_charity_related = new_request.requestConfig.isCharityRelated
    ...
    generated_image_urls, local_image_paths = generate_image_assets(
        ...
        clothing=clothing,
        theme=theme,
        background=background_color if background_color else background,
    ...
    primary_font_path = os.path.join(LOCAL_INPUT_DIR_FONTS, "Gochi_Hand/GochiHand-Regular.ttf")
    secondary_font_path = os.path.join(LOCAL_INPUT_DIR_FONTS, "AlbertSans/AlbertSans-SemiBold.ttf")
    ...
    try:
        if not template or not layouts:
            logger.error(
                "No template or layout found. skipping post generation ..."
            )
            return
        ...
        for idx, img_file_name in enumerate(local_image_paths):
            ...
            process_image(img_file_name, input_text, request_id, idx, aspect_ratio, layouts, primary_font_path, secondary_font_path, disclaimer_and_sign_off, generated_image_urls, user_id)
