...
from utils.constants import (GCS_INPUT_BUCKET_ROOT, GCS_INPUT_DIR_TEMPLATES,
                             GCS_OUTPUT_DIR_GENERATED_IMAGES,
                             GCS_OUTPUT_DIR_POSTS,
                             IMAGE_GENERATION_INPUT_PARAMS_GENERATION_PROMPT,
                             LOCAL_INPUT_DIR_FONTS, LOCAL_INPUT_DIR_GRAPHICS,
                             LOCAL_TEMP_DIR, NEGATIVE_PROMPT_FOR_IMAGEN,
...
def get_post_template_by_name(template_name: str) -> Template:
    try:
        logger.info(f"Retrieving post template: {template_name}")
...
def update_request_count_in_db(request_id: str, requestConfig: RequestConfig, countOfImagesGenerated: int) -> None:
...
def update_request_status_in_db(request_id: str, status: RequestStatus) -> None:
    logger.info(f"Updating request status in request {request_id} in db to {status}")
    try:
        collection_requests.document(request_id).set({"status": status}, 
            merge=True)
    except Exception as e:
        logger.error(f"An error occurred while updating request in db: {e}")
...
class transparent_background_remover:
    def __init__(self):
        self.remover = Remover(device='cuda:0')

    def remove_background(self, input_path, output_path, mask_path):
        image = Image.open(input_path).convert("RGB")
        output_image = self.remover.process(image)
        output_image.save(output_path)
        mask_image = self.remover.process(image, type="map")
        mask_image.save(mask_path)

remover = transparent_background_remover()

def remove_background_with_transparent_background(input_path, output_path, mask_path):
    logger.info(f"Removing background from {input_path} with transparent-background")
    start_time = time.time()
    remover.remove_background(input_path, output_path, mask_path)
    end_time = time.time()
    logger.info(f"Time taken to remove background with transparent-background: {end_time - start_time} seconds")

DEFAULT_ALPHA_MATTING_PARAMS = {
    "alpha_matting_foreground_threshold": 150,
    "alpha_matting_background_threshold": 35,
    "alpha_matting_erode_size": 25,
}

# Deprecated - this function uses the rembg model, we are using transparent-background model instead as it has better masking accuracy
def remove_background(
    input_path, output_path, mask_path, margin=10, alpha_threshold=10, alpha_matting_params=DEFAULT_ALPHA_MATTING_PARAMS
):
    ...
    output_data = remove(
        ...
        session=session,
        alpha_matting=True,
        alpha_matting_foreground_threshold=alpha_matting_params["alpha_matting_foreground_threshold"],
        alpha_matting_background_threshold=alpha_matting_params["alpha_matting_background_threshold"],
        alpha_matting_erode_size=alpha_matting_params["alpha_matting_erode_size"],
    )
...
def create_gradient_for_signoff_text(image_path, output_path):
    image = Image.open(image_path).convert("RGBA")
    img_width, img_height = image.size
    overlay = Image.new("RGBA", image.size, (255, 255, 255, 0))

    gradient_height = 240  # Adjust gradient height
    gradient = Image.new("RGBA", (img_width, gradient_height))
    pixels = gradient.load()

    for x in range(img_width):
        for y in range(gradient_height):
            alpha = int((y / gradient_height) * 200)  # Linear alpha increase
            pixels[x, y] = (255, 255, 255, alpha)  # White with increasing alpha

    overlay.paste(gradient, (0, img_height - gradient_height))
...
def create_marketing_banner_baseline(
    ...
    logger.info(f"OVERLAYING TEXT ON IMAGE")
    logger.info(f"Background config: {background_config}")
    
    if "actor_position" in background_config:
        # Process Actor overlay - actor image is the image generated by Imagen3
    ...
    if "text_tagline_position" in background_config:
        if is_background_needed:
            create_gradient_for_signoff_text(output_path, output_path)
    ...
    if "text_action_position" in background_config:
        if is_background_needed:
            badge_overlay_config = {  # allows out of bounds stickers
                ...
                "y": background_config["text_action_position"]["y"] - 70,
...
def generate_imagen_outputs(
    ...
    while retry_count < 3:
        logger.info(
            f"Generating {number_of_images} images with aspect ratio {aspect_ratio} and model {model}, attempt {retry_count+1}"
        )
...
def generate_image_assets(
    subject, age, clothing, theme, background, photography, request_id, aspect_ratio, image_count=4
):
    ...
    logger.info(f"injected prompt: {'''**Important**: As the image generated will be masked to remove background, make sure to leave sufficient negative space around the subjects and position them in the center of the image. It is acceptable to frame the image from the waist up, or even closer, to prioritize full inclusion of the subjects. Ensure that the generated image is optimised for masking the subject out from the background. If there are people in the image, ensure that there are less than 4 people.''' if aspect_ratio != AspectRatio.plain_square and aspect_ratio != AspectRatio.full_image else 'NO PROMPT INJECTED'}")
    ...
    prompt_rewrite = f"""
        ...
        To meet the branding guidelines, ensure that highly saturated red with minimal blue or yellow undertones (hex color code #E2001A) is incorporated into the image tastefully, adhering to a ratio of 30% for simpler illustration and less than 30% for complex images.
        ...
        Ensure that the generated image is optimised for masking the subject out from the background. If there are people in the image, ensure that there are less than 4 people.''' if aspect_ratio != AspectRatio.plain_square and aspect_ratio != AspectRatio.full_image else ''}
    ...
    logger.info(f"Images upload to GCS complete")
...
def generate_platform_specific_instructions(platform: SocialMediaPlatform) -> str:
    ...
    if platform == SocialMediaPlatform.instagram:
        instructions = (
            "Given that this caption is meant for Instagram, keep the caption concise and impactful, as Instagram users tend to scroll quickly. "
            "Optimize for visual engagement. Consider using relevant emojis sparingly."
            "Include a clear call to action"
        )
    elif platform == SocialMediaPlatform.facebook:
        instructions = (
            "Given that this caption is meant for Facebook, maintain a slightly more formal tone than Instagram.  Use a clear call to action "
            "Facebook posts can be a bit longer, so feel free to provide more context."
        )
    elif platform == SocialMediaPlatform.linkedin:
        instructions = (
            "Given that this caption is meant for LinkedIn, maintain a professional tone for the professional audience on LinkedIn. Use relevant industry keywords to improve searchability. "
            "Craft a compelling headline, avoid long blocks of text. Consider a call to action (including the one in the user input) to comment, like your post, answer a question, or share it with someone who could benefit from reading the post."
        )
    elif platform == SocialMediaPlatform.x:
        instructions = (
            "Given that this caption is meant for X/Twitter, keep the caption concise and impactful. Ensure that the caption is *strictly* within 280 characters to respect the platform's character limit."
            "Use relevant hashtags strategically to improve visibility.  Include a clear call to action "
        )
    else:
        instructions = "Use a general tone and style appropriate for broad social media use."
...
def generate_post_caption(post_text_prompt_input, social_media_platform: SocialMediaPlatform = SocialMediaPlatform.instagram):
    ...
    while retry_count < 3 and caption == "":
        logger.info(f"Generating caption for {social_media_platform.value}, iteration {retry_count+1}")
        try:
            prompt = POST_TEXT_CAPTION_TEMPLATE.format(user_input=post_text_prompt_input, social_media_platform=social_media_platform.value.upper())
            prompt = f"{prompt}\n**Important**: Platform Specific Instructions:\n{generate_platform_specific_instructions(social_media_platform)}"
            response = model.generate_content(
                [prompt],
                generation_config=generation_config,
            )
...
def convert_template_to_snake_case(template: Template) -> dict:
    ...
    return {camel_to_snake(key): value for key, value in template.model_dump(exclude_none=True).items()}
...
def aspect_ratio_to_template(aspect_ratio):
    ...
    elif aspect_ratio == AspectRatio.plain_square:
        return "SQUARE_PLAIN_SQUARE_TEMPLATE_1"
...
def process_image(
    ...
    user_id,
    social_media_platform: SocialMediaPlatform = SocialMediaPlatform.instagram,
):
    ...
    is_background_needed = aspect_ratio == AspectRatio.full_image or aspect_ratio == AspectRatio.plain_square
    if not is_background_needed:
        # remove_background(img_file_name, no_bg_image_path, mask_path)
        remove_background_with_transparent_background(img_file_name, no_bg_image_path, mask_path)
        logger.info(f"removed background from {img_file_name}")

    with ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(generate_post_text, input_text),
            executor.submit(generate_post_caption, input_text, social_media_platform),
        ]
        headline, event_details, call_to_action = futures[0].result()
        caption = futures[1].result()
...
def format_background_color_description(background_color, art_style, aspect_ratio):
    if aspect_ratio == AspectRatio.full_image:
        return f"choose a engaging and interesting background that has a mostly {'hex code ' + background_color if background_color else 'pastel'} color (if user specified any background details in the description, use that instead)"
    elif art_style == "Vector Art":
        return "plain white background"
    elif aspect_ratio == AspectRatio.plain_square:
        return f"engaging and interesting background suitable for the subject, optimised for social media impact"
    else:
        return f"plain studio background with a white screen"
            ...
            while retries < max_retries:
                try:
                    ...
                    if retries == max_retries:
                        ...
                        request_id = args[0]
                        update_request_status_in_db(request_id, RequestStatus.error)
                    logger.warning(
...
def generate_posts_background(request_id, user_id, request_config) -> None:
    #check if status of request is pending
    request = collection_requests.document(request_id).get()
    request_dict = request.to_dict()
    logger.info(f"request: {request_dict}")
    if request_dict["status"] != RequestStatus.pending:
        logger.info(f"Request {request_id} is not in pending status, skipping generation. Request status: {request_dict['status']}")
        return
    ...
    social_media_platform = request_config["socialMediaPlatform"] or 'instagram'
    ...
    logger.info(f"Retrieving post template for aspect ratio: {aspect_ratio}")
    template = get_post_template_by_name(aspect_ratio_to_template(aspect_ratio))
    logger.info(f"Template retrieved: {template}")
    ...
    try:
        ...
        if len(local_image_paths) != post_count:
            update_request_count_in_db(request_id, request_config, len(local_image_paths))
...
#     parser.add_argument('mask_path', help='Path for mask image')
#     parser.add_argument('--margin', type=int, default=10, help='Margin size (default: 10)')
#     parser.add_argument('--alpha_threshold', type=int, default=10, help='Alpha threshold (default: 10)')
#     parser.add_argument('--alpha_matting_foreground_threshold', type=int, default=150, help='Alpha matting foreground threshold (default: 10)')
#     parser.add_argument('--alpha_matting_background_threshold', type=int, default=10, help='Alpha matting background threshold (default: 10)')
#     parser.add_argument('--alpha_matting_erode_size', type=int, default=25, help='Alpha matting erode size (default: 6)')
